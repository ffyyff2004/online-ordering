...
public class SmartMealController extends BaseController {
    ...
    private String callModel(String username, String requirement, List<Foods> foods) throws Exception {
        ...
        for (Foods food : foods) {
            menu.append("餐品编号:").append(food.getFoodsid())
                    .append("|名称:").append(food.getFoodsname())
                    .append("|分类:").append(food.getCatename())
                    .append("|价格:").append(food.getPrice())
                    .append("|介绍:").append(food.getContents()).append("\n");
        }
        String system = "你是在线点餐系统的智能选餐助手。只能从用户提供的当前可售餐品中推荐，不能创造不存在的餐品。"
                + "必须返回严格 JSON，不要 Markdown，不要额外文字。JSON 格式为 {\"recommendations\":["
                + "{\"foodsid\":\"餐品编号\",\"reason\":\"结合用户需求的简短理由\"},"
                + "{\"foodsid\":\"餐品编号\",\"reason\":\"理由\"},"
                + "{\"foodsid\":\"餐品编号\",\"reason\":\"理由\"}]}。"
                + "三个方案必须是不同餐品，并且都满足用户需求。";
        String user = "用户称呼:" + username + "\n用户需求:" + requirement + "\n当前可售餐品:\n" + menu;
