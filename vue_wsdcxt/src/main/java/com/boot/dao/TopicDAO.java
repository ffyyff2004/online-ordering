...
public interface TopicDAO {
	...
	*/
	...
	// 插入订单评价表数据 调用mapper包topic.xml里的insertTopic配置 返回值0(失败),1(成功)
	public int insertTopic(Topic topic);

	// 更新订单评价表数据 调用mapper包topic.xml里的updateTopic配置 返回值0(失败),1(成功)
	public int updateTopic(Topic topic);

	// 按主键删除订单评价表数据 调用mapper包topic.xml里的deleteTopic配置 返回值0(失败),1(成功)
	public int deleteTopic(String topicid);

	// 批量删除订单评价表数据 调用mapper包topic.xml里的deleteTopicByIds配置 返回值0(失败),大于0(成功)
	public int deleteTopicByIds(String[] ids);

	// 查询订单评价表全部数据 调用mapper包topic.xml里的getAllTopic配置 返回List<Topic>类型的数据
	...
	// 按主键查询订单评价表返回单一的Topic实例 调用mapper包topic.xml里的getTopicById配置
