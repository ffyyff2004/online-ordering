...
public interface OrdersDAO {
	...
	*/
	...
	// 插入订单表数据 调用mapper包orders.xml里的insertOrders配置 返回值0(失败),1(成功)
	public int insertOrders(Orders orders);

	// 更新订单表数据 调用mapper包orders.xml里的updateOrders配置 返回值0(失败),1(成功)
	public int updateOrders(Orders orders);

	// 按主键删除订单表数据 调用mapper包orders.xml里的deleteOrders配置 返回值0(失败),1(成功)
	public int deleteOrders(String ordersid);

	// 批量删除订单表数据 调用mapper包orders.xml里的deleteOrdersByIds配置 返回值0(失败),大于0(成功)
	public int deleteOrdersByIds(String[] ids);

	// 查询订单表全部数据 调用mapper包orders.xml里的getAllOrders配置 返回List<Orders>类型的数据
	...
	// 按主键查询订单表返回单一的Orders实例 调用mapper包orders.xml里的getOrdersById配置
