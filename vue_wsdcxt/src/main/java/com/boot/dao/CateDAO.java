...
public interface CateDAO {
	...
	*/
	...
	// 插入食品类型表数据 调用mapper包cate.xml里的insertCate配置 返回值0(失败),1(成功)
	public int insertCate(Cate cate);

	// 更新食品类型表数据 调用mapper包cate.xml里的updateCate配置 返回值0(失败),1(成功)
	public int updateCate(Cate cate);

	// 按主键删除食品类型表数据 调用mapper包cate.xml里的deleteCate配置 返回值0(失败),1(成功)
	public int deleteCate(String cateid);

	// 批量删除食品类型表数据 调用mapper包cate.xml里的deleteCateByIds配置 返回值0(失败),大于0(成功)
	public int deleteCateByIds(String[] ids);

	// 查询食品类型表全部数据 调用mapper包cate.xml里的getAllCate配置 返回List<Cate>类型的数据
	...
	// 按主键查询食品类型表返回单一的Cate实例 调用mapper包cate.xml里的getCateById配置
